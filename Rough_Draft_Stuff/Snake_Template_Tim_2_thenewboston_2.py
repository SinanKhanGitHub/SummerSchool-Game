...
difficulty = 1
img_apple = pygame.transform.scale(pygame.image.load('apple.jpg'),((width//rows),(width//rows)))
...
class cube(object):
    ...
    def draw(self, surface, eyes=False):
        ...
        #pygame.draw.ellipse(surface, self.color, (i*dis+1,j*dis+1, dis+3, dis-2), 0) # Its been stretched out by 3, but problems remain with keeping the 'cubes' all together
...
def drawGrid(w, rows, surface):
    ...
    for l in range(rows):
        ...
        pygame.draw.line(surface, pastel_green, (x, w * ratio_header_to_screen),(x,w-sizeBtwn*difficulty))     # Colors changed to white 0,0,0
        pygame.draw.line(surface, pastel_green, (sizeBtwn*difficulty, y + w * ratio_header_to_screen - sizeBtwn),(w - sizeBtwn*difficulty, y + w * ratio_header_to_screen  - sizeBtwn))
        

def redrawWindow(surface):
    global s, snack  # Dont need rows or width, ratio_header_to_screen 
    #ratio_header_to_screen = 0.2 # Added because of New Boston
    border = width // rows # ratio'd to number of rows
    surface.fill(pastel_green, rect=[border,width * ratio_header_to_screen, width - border, width - width * ratio_header_to_screen])
    surface.fill(emerald,rect=[0,0, width, width * ratio_header_to_screen]) # Trying the big box
    surface.fill(emerald,rect=[0,width-border, width, border])
    surface.fill(emerald,rect=[0,width * ratio_header_to_screen, border, width - width * ratio_header_to_screen])
    surface.fill(emerald,rect=[width-border,width * ratio_header_to_screen, border, width - width * ratio_header_to_screen]) # Bottom barrier New Boston
    s.draw(surface)
    snack.draw(surface) 
...
def start_screen(): #Taken from New Boston
    ...
    while intro:
        win.fill(green)
        centered_txt2_screen("Welcome to Sinan's Snake Adventure",red,3)
        centered_txt2_screen("How big can your snake get?",red,5)
        centered_txt2_screen("Can you get the apples while avoiding the perils that lie?",red,7)
        centered_txt2_screen("Press C to play or L_Shift and Q to quit",red,11)
        ...
        for event in pygame.event.get():
            ...
            for key in keys:
                if keys[pygame.K_c]:
                    main()
                    intro = False
                elif keys[pygame.K_LSHIFT and pygame.K_q]:
                    pygame.quit()
                    intro = False
                    break
...
def main():
    global width, s, snack #rows
    score = 0
    increasing_score = 1
    #width = 750   # Changed from 500 * 20 to 1000 * 40 to 750 * 30
    #rows = 30
    #win = pygame.display.set_mode((width, width)) better to have win outside
    s = snake(purple, (rand('X'),rand('Y'))) # Trying something new with the position, definitely cant be at x = 0, max(rows) or y = 0, max(rows) as then it is off the screen (rows - rows // rows,rows - rows // rows)
    ...
    while gameOn:
        FPS = 30 # Try fluid movement
        pygame.time.delay(0)       # Changed to 0 from 50
        clock.tick(FPS)       # Changed to 40 from 10
        s.move()
        #s_2.move()  # Adding second snake
        #print(s.body[0].pos)
        print(len(s.body))
        if gameOver == True: # Added by the new boston to allow for game over 
            s.crash()
            centered_txt2_screen("Game over, press C to play again or Q to Quit", green, 2, size = bubblefont)
            centered_txt2_screen("Press C to play again or Q to Quit", green, 10, size = medfont)
            centered_txt2_screen("Press C to play again or Q to Quit", green, 15, size = smallfont)
            pygame.display.update()
        ...
        elif s.body[0].pos[1] >= rows - (rows // rows): # Added by new boston, 
            #some issue with being able to leave at the bottom of the screen
            gameOver = True
            continue
